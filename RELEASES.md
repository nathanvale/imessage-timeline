...
- “Blocked because branch is out of date”: update with main and rerun checks: `git fetch origin && git merge origin/main` (or `git rebase origin/main`), then `gh pr checks <#>`; if still blocked, run `scripts/pr-blockers.sh <#>`.
- “No unpublished projects to publish” during canary/publish: add a changeset (or code change with `bunx changeset`) so there’s something to release; then rerun.
